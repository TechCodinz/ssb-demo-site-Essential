...
from api.routers.auth import users_db
from api.routers.license import licenses_db

router = APIRouter()
security = HTTPBearer()

# Mock cloud sessions (replace with actual tracking)
cloud_sessions = {}


@router.get("/cloud-status")
async def get_cloud_status(credentials: HTTPAuthorizationCredentials = Depends(security)):
...
@router.get("/billing")
async def get_billing(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    try:
        ...
        for key, data in licenses_db.items():
            if data.get("email") == email:
                ...
                return {
                    "plan": data["plan"],
                    "renewal_date": data["expires"],
                    "days_until_renewal": max(0, days_left),
                    "auto_renew": False,  # TODO: Implement
